...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addUserOption(opt => opt.setName('membre').setDescription('Membre à modifier').setRequired(true))
        .addIntegerOption(opt => opt.setName('id').setDescription('Nouvel ID database').setRequired(true).setMinValue(1))
        .setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator),
].map(cmd => cmd.toJSON());
...
async function closeDrop(drop_id) {
    ...
    eligible = eligible.filter(uid => {
        const member = client.guilds.cache.get(GUILD_ID)?.members.cache.get(uid);
        return member && !isServerBooster(member);
    });
...
async function handleIdModify(interaction) {
    ...
    const conflict = db.prepare('SELECT user_id FROM members WHERE circle_id = ? AND user_id != ?').get(newId, member.id);
    if (conflict) {
        return await interaction.editReply({ content: `Le Circle ID #${padId(newId)} est déjà utilisé par <@${conflict.user_id}>.` });
    }
    ...
    const roleName = `CIRCLE ID — #${padId(newId)}`;
    let newRole = interaction.guild.roles.cache.find(role => role.name === roleName);
    if (!newRole) {
        newRole = await interaction.guild.roles.create({
            name: roleName,
            color: 0x5865F2,
            mentionable: false,
            reason: 'Modification manuelle du Circle ID database'
        }).catch(() => null);
    }
