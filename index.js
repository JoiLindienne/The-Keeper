 ...
 */
async function syncCircleSystems(guild) {
    let checked = 0;
    let circle = 0;
...
client.on('interactionCreate', async interaction => {
    try {
        if (interaction.isChatInputCommand()) {
            if (['drop', 'signal', 'chosen-one', 'sync-circle'].includes(interaction.commandName)) {
            ...
            if (interaction.commandName === 'drop' && interaction.options.getSubcommand() === 'create') {
                ...
                const { checked, circle } = await syncCircleSystems(interaction.guild);
