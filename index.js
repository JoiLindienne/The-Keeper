...
const circleMemberTasks = new Map();
let lastFullMemberFetch = 0;
let circleInitializationPromise = null;
...
const GATE_CHANNEL_ID = '1522567182939197541';
const CIRCLE_ID_ROLE_PATTERN = /^CIRCLE ID — #(\d+)$/;
const WELCOME_TEMPLATES = [
    "Une nouvelle place vient d’être accordée. Bienvenue dans le Cercle, {member}.",
    "Le Cercle s’agrandit. Bienvenue, {member}. Fais comme si tout ceci était parfaitement normal.",
    "La Gate vient de murmurer un nouveau nom : {member}. Entre, on t’attendait presque.",
    "Le dossier {circleId} vient d’être déclassifié. Bienvenue parmi nous, {member}.",
    "Quelqu’un a encore laissé la porte entrouverte… Bienvenue dans le Cercle, {member}.",
    "La Lune a validé ta demande sans lire les petites lignes. Bienvenue, {member}.",
    "Un siège vide ne l’est plus. Prends place, {member}, et ne pose pas trop de questions.",
    "Le silence vient officiellement d’accueillir {member}. Le Cercle aussi.",
    "Les archives ont bougé toutes seules : {member} appartient désormais au Cercle.",
    "Le rituel a fonctionné. Pour une fois. Bienvenue, {member}.",
    "La Gate reconnaît {member}. Nous nierons évidemment avoir organisé tout ça.",
    "Un nouveau secret vient d’obtenir un nom : {member}. Bienvenue dans le Cercle.",
    "La place {circleId} est désormais occupée par {member}. Coïncidence ? Certainement pas.",
    "Le Cercle compte une ombre de plus. Bienvenue parmi les initiés, {member}.",
    "On avait dit « pas de nouveaux témoins ». Tant pis : bienvenue, {member}.",
    "Les bougies ont vacillé au bon moment. Bienvenue dans le Cercle, {member}.",
    "Notification interne : {member} a franchi la Gate. Personne ne panique.",
    "Le protocole secret vient d’ajouter {member}. Le protocole public, lui, n’existe pas.",
    "Le Cercle t’ouvre une porte, {member}. Évite juste celle du fond.",
    "Quelque chose s’est réveillé dans les archives. Ah non, c’est seulement {member}. Bienvenue."
];
...
function hasCoreTag(member) {
    ...
    return typeof tag === 'string' && tag.toUpperCase() === 'COCO';
}
...
function memberMatchesDropTag(member, tagReq) {
    ...
    // Pour l’instant le système Circle Drop utilise le tag serveur COCO.
    // Si un autre tag est écrit par erreur, on le refuse au lieu de valider tout le monde.
    if (requiredTag !== 'COCO') return false;
...
function isCircleOwner(member) {
    return Boolean(member?.guild?.ownerId && member.id === member.guild.ownerId);
}

function getCircleIdsFromRoles(member) {
    if (!member?.roles?.cache) return [];

    return member.roles.cache
        .map(role => role.name.match(CIRCLE_ID_ROLE_PATTERN))
        .filter(Boolean)
...
function isServerBooster(member) {
    ...
    // Discord inclut toujours @everyone dans la liste des rôles d'un membre.
    // On ignore uniquement les membres dont le rôle Server Booster est le seul
    // rôle en plus de @everyone. Un booster ayant un autre rôle reste considéré.
    const assignedRoles = roles.filter(role => role.id !== member.guild.id);

    return assignedRoles.size === 1 &&
        assignedRoles.first()?.tags?.premiumSubscriberRole === true;
}

function getHighestKnownCircleId(guild) {
    const dbMax = db
        .prepare('SELECT MAX(circle_id) AS max_id FROM members WHERE circle_id >= 0')
        .get()?.max_id ?? 0;

    const roleMax = guild.roles.cache.reduce((highest, role) => {
        const match = role.name.match(CIRCLE_ID_ROLE_PATTERN);
        return match ? Math.max(highest, Number(match[1])) : highest;
    }, 0);

    return Math.max(dbMax, roleMax);
}
...
/**
 * Réserve l'ID avant toute opération Discord asynchrone. Deux arrivées proches
 * ne peuvent ainsi plus s'inverser à cause du temps de création des rôles.
 */
function reserveCircleIdForMember(member) {
    if (!member || member.user.bot) return null;
    if (!isCircleOwner(member) && isServerBooster(member)) return null;
    ...
    const reserve = db.transaction(() => {
        ...
        if (isCircleOwner(member)) {
            if (existing?.circle_id === 0) return 0;

            const occupant = db
                .prepare('SELECT user_id FROM members WHERE circle_id = 0 AND user_id != ?')
                .get(member.id);

            if (occupant) {
                const archiveId = (db
                    .prepare('SELECT MIN(circle_id) AS min_id FROM members')
                    .get()?.min_id ?? 0) - 1;
                db.prepare('UPDATE members SET circle_id = ? WHERE user_id = ?')
                    .run(Math.min(-1, archiveId), occupant.user_id);
            }

            if (existing) {
                db.prepare('UPDATE members SET circle_id = 0 WHERE user_id = ?').run(member.id);
            } else {
                db.prepare(`
                    INSERT INTO members (user_id, circle_id, join_date)
                    VALUES (?, 0, ?)
                `).run(member.id, getMemberJoinDate(member));
            }
            return 0;
        }

        if (existing && existing.circle_id >= 1) {
            return existing.circle_id;
        }

        const nextCircleId = getHighestKnownCircleId(member.guild) + 1;
        if (existing) {
            db.prepare('UPDATE members SET circle_id = ? WHERE user_id = ?')
                .run(nextCircleId, member.id);
        } else {
            db.prepare(`
                INSERT INTO members (user_id, circle_id, join_date)
                VALUES (?, ?, ?)
            `).run(member.id, nextCircleId, getMemberJoinDate(member));
        }
...
/**
 * Au premier démarrage après la correction, les rôles existants sont conservés
 * comme ordre historique. La base est réalignée dessus, sans réutiliser les
 * numéros vides laissés par d'anciens membres.
 */
function reconcileExistingCircleIds(guild) {
    const owner = guild.members.cache.get(guild.ownerId);
    const alreadyReconciled = db
        .prepare("SELECT value FROM bot_state WHERE key = 'circle_id_reconciliation_v2'")
        .get()?.value === 'done';

    if (alreadyReconciled) {
        if (owner && !owner.user.bot) reserveCircleIdForMember(owner);
        return 0;
    }
    ...
    if (owner && !owner.user.bot) {
        assignments.push({ member: owner, circleId: 0 });
        usedIds.set(0, owner.id);
    }

    const membersWithRoles = [...guild.members.cache.values()]
        .filter(member => !member.user.bot && member.id !== guild.ownerId)
        .sort((a, b) =>
            (a.joinedTimestamp || Number.MAX_SAFE_INTEGER) -
            (b.joinedTimestamp || Number.MAX_SAFE_INTEGER)
        );

    for (const member of membersWithRoles) {
        const currentIds = getCircleIdsFromRoles(member).filter(id => id > 0);
        if (currentIds.length === 0) continue;

        const dbId = db
            .prepare('SELECT circle_id FROM members WHERE user_id = ?')
            .get(member.id)?.circle_id;
        const preferredId = currentIds.includes(dbId) ? dbId : currentIds[0];

        if (!usedIds.has(preferredId)) {
            assignments.push({ member, circleId: preferredId });
            usedIds.set(preferredId, member.id);
        } else {
            console.warn(
                `[CIRCLE ID] Le rôle #${padId(preferredId)} est partagé par plusieurs membres. ` +
                `${member.user.tag} recevra le prochain ID libre.`
            );
        }
    }

    const assignedUsers = new Set(assignments.map(({ member }) => member.id));
    const assignedIds = new Set(assignments.map(({ circleId }) => circleId));

    const reconcile = db.transaction(() => {
        let temporaryId = Math.min(
            -1,
            (db.prepare('SELECT MIN(circle_id) AS min_id FROM members').get()?.min_id ?? 0) - 1
        );

        for (const { member } of assignments) {
            if (db.prepare('SELECT 1 FROM members WHERE user_id = ?').get(member.id)) {
                db.prepare('UPDATE members SET circle_id = ? WHERE user_id = ?')
                    .run(temporaryId--, member.id);
            }
        }

        for (const circleId of assignedIds) {
            const conflict = db
                .prepare('SELECT user_id FROM members WHERE circle_id = ?')
                .get(circleId);
            if (conflict && !assignedUsers.has(conflict.user_id)) {
                db.prepare('UPDATE members SET circle_id = ? WHERE user_id = ?')
                    .run(temporaryId--, conflict.user_id);
            }
        }

        for (const { member, circleId } of assignments) {
            db.prepare(`
                INSERT INTO members (user_id, circle_id, join_date)
                VALUES (?, ?, ?)
                ON CONFLICT(user_id) DO UPDATE SET circle_id = excluded.circle_id
            `).run(member.id, circleId, getMemberJoinDate(member));
        }

        db.prepare(`
            INSERT INTO bot_state (key, value)
            VALUES ('circle_id_reconciliation_v2', 'done')
...
async function syncMemberCircleIdRole(member, circleId, reason = 'Synchronisation du Circle ID') {
    const roleName = `CIRCLE ID — #${padId(circleId)}`;
    let role = member.guild.roles.cache.find(currentRole => currentRole.name === roleName);
    ...
    if (!role) {
        try {
            ...
            console.error(`[CIRCLE ID] Impossible de créer ${roleName}:`, error?.message || error);
            return false;
        }
    }

    try {
        // On ajoute d'abord le bon rôle : le membre ne se retrouve jamais sans ID.
        if (!member.roles.cache.has(role.id)) {
            await member.roles.add(role, reason);
        }

        const oldRoles = member.roles.cache.filter(currentRole =>
            CIRCLE_ID_ROLE_PATTERN.test(currentRole.name) &&
        ...
        console.error(
            `[CIRCLE ID] Impossible d'attribuer ${roleName} à ${member.user.tag}:`,
            error?.message || error
        );
        return false;
    }
...
async function ensureCircleMemberUnlocked(member, allowWithoutStatus = false) {
    if (!member || member.user.bot) return null;
    if (!isCircleOwner(member) && isServerBooster(member)) return null;
    if (!isCircleOwner(member) && !allowWithoutStatus && !hasCircleStatusRole(member)) return null;

    const circleId = reserveCircleIdForMember(member);
    if (circleId === null) return null;

    await syncMemberCircleIdRole(member, circleId);
    return circleId;
}

async function ensureCircleMember(member, options = {}) {
    if (!member) return null;

    const previousTask = circleMemberTasks.get(member.id) || Promise.resolve();
    const currentTask = previousTask
        .catch(() => {})
        .then(() => ensureCircleMemberUnlocked(member, options.allowWithoutStatus === true));

    circleMemberTasks.set(member.id, currentTask);

    try {
        return await currentTask;
    } finally {
        if (circleMemberTasks.get(member.id) === currentTask) {
            circleMemberTasks.delete(member.id);
        }
    }
}

async function checkAndAssignCircleStatusRoles(member) {
    if (!member || (!isCircleOwner(member) && isServerBooster(member))) return;
    ...
    try {
        ...
        console.error(`[TAG COCO] Impossible de synchroniser ${member.user.tag}:`, error?.message || error);
    }
}

function getNextWelcomeTemplate() {
    const choose = db.transaction(() => {
        const previousRow = db
            .prepare("SELECT value FROM bot_state WHERE key = 'last_welcome_template'")
            .get();
        const previousIndex = Number(previousRow?.value);
        const candidates = WELCOME_TEMPLATES
            .map((_, index) => index)
        ...
        db.prepare(`
            INSERT INTO bot_state (key, value)
            VALUES ('last_welcome_template', ?)
            ON CONFLICT(key) DO UPDATE SET value = excluded.value
        `).run(String(nextIndex));

        return WELCOME_TEMPLATES[nextIndex];
    });
...
async function welcomeGate(member, circle_id) {
    ...
    const gate = member.guild.channels.cache.get(GATE_CHANNEL_ID) ||
        await member.guild.channels.fetch(GATE_CHANNEL_ID).catch(() => null);

    if (!gate || gate.type !== ChannelType.GuildText) {
        console.error(`[WELCOME] Le salon gate ${GATE_CHANNEL_ID} est introuvable ou n'est pas textuel.`);
        return;
    }

    const message = getNextWelcomeTemplate()
        .replaceAll('{member}', `<@${member.id}>`)
        .replaceAll('{circleId}', `#${padId(circle_id)}`);
    ...
    try {
        ...
        console.error(`[WELCOME] Impossible d'accueillir ${member.user.tag}:`, error?.message || error);
    }
}
...
async function handleProfile(interaction) {
    ...
    if (!isCircleOwner(member) && isServerBooster(member)) {
    ...
    const row = db.prepare('SELECT * FROM members WHERE user_id = ?').get(user.id);
    // Statut In/Outside
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(0x5865F2)
        .addFields(
            { name: 'Circle ID', value: `#${padId(row.circle_id)}`, inline: true },
...
function getMemberCircleData(member) {
    ...
    const circleId = row?.circle_id ?? null;
    ...
    for (let i = 0; i < members.length; i += 5) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(chunk.map(({ member, data }) => {
                const id = data.circleId !== null ? `#${padId(data.circleId)}` : 'Aucun';
...
async function handleIdModify(interaction) {
    ...
    if (isCircleOwner(member) && newId !== 0) {
        return await interaction.reply({ content: "L'owner doit toujours conserver le Circle ID #000.", ephemeral: true });
    }

    if (!isCircleOwner(member) && newId === 0) {
        return await interaction.reply({ content: "Le Circle ID #000 est réservé à l'owner.", ephemeral: true });
    }

    if (!isCircleOwner(member) && !hasCircleStatusRole(member)) {
...
async function applyCircleIdRole(member, circleId, guild, reason = 'Modification du Circle ID') {
    if (member.guild.id !== guild.id) return false;
    return syncMemberCircleIdRole(member, circleId, reason);
}
...
async function syncCircleSystems(guild, forceFullFetch = false) {
    const fullFetchCompleted = await fetchAllMembersSafely(guild, forceFullFetch);
    const reconciliationPending = db
        .prepare("SELECT value FROM bot_state WHERE key = 'circle_id_reconciliation_v2'")
        .get()?.value !== 'done';

    if (reconciliationPending) {
        if (!fullFetchCompleted) {
            console.error(
                '[CIRCLE ID] Migration reportée : la liste complète des membres est indisponible.'
            );
            return { checked: 0, circle: 0 };
        }

        const reconciledMembers = reconcileExistingCircleIds(guild);
        console.log(
            `[CIRCLE ID] ${reconciledMembers} attribution(s) existante(s) ` +
            'préservée(s) et réalignée(s).'
        );
    }

    let checked = 0;
    let circle = 0;
    const eligibleMembers = [...guild.members.cache.values()]
        .filter(member =>
            !member.user.bot &&
            (isCircleOwner(member) || !isServerBooster(member))
        )
        .sort((a, b) =>
            (a.joinedTimestamp || Number.MAX_SAFE_INTEGER) -
            (b.joinedTimestamp || Number.MAX_SAFE_INTEGER)
    ...
    // Si plusieurs membres sont arrivés pendant que le bot était hors ligne,
    // leurs ID sont réservés selon leur vraie date d'arrivée avant tout appel réseau.
    for (const member of eligibleMembers) {
        reserveCircleIdForMember(member);
    }
...
client.once('ready', async () => {
    ...
    const initialCircleSync = (async () => {
        await registerCommands();
        const guild = client.guilds.cache.get(GUILD_ID) || await client.guilds.fetch(GUILD_ID);
        await syncCircleSystems(guild, true);
    })();

    circleInitializationPromise = initialCircleSync;
    try {
        await initialCircleSync;
    } finally {
        if (circleInitializationPromise === initialCircleSync) {
            circleInitializationPromise = null;
        }
    }
    // Drops
...
client.on('guildMemberAdd', async member => {
    ...
    if (!isCircleOwner(member) && isServerBooster(member)) return;

    if (circleInitializationPromise) {
        await circleInitializationPromise.catch(error => {
            console.error('[CIRCLE ID] Synchronisation initiale incomplète:', error?.message || error);
        });
    }

    // Réservation synchrone avant tout appel Discord : l'ordre des événements
    // d'arrivée reste l'ordre des Circle ID.
    reserveCircleIdForMember(member);
    await checkAndAssignCircleStatusRoles(member);
    const circleId = await ensureCircleMember(member, { allowWithoutStatus: true });
    if (circleId !== null) await welcomeGate(member, circleId);
});

client.on('guildMemberUpdate', async (oldM, newM) => {
    if (newM.user.bot) return;
    if (!isCircleOwner(newM) && isServerBooster(newM)) return;
    await newM.user.fetch(true).catch(() => {});
    await checkAndAssignCircleStatusRoles(newM);
    await ensureCircleMember(newM);
});
            ...
            if (interaction.commandName === 'drop' && interaction.options.getSubcommand() === 'create') {
                ...
                const modal = new ModalBuilder()
                    ...
                    .addComponents(
                        ...
                        new ActionRowBuilder().addComponents(
                            new TextInputBuilder().setCustomId('requirements').setLabel('TAG=COCO | CONDITIONS=... (format)').setStyle(TextInputStyle.Paragraph).setRequired(false)
                        ),
                    );
                await interaction.showModal(modal);
                ...
                const modal = new ModalBuilder()
                    ...
                    .addComponents(
                        ...
                        new ActionRowBuilder().addComponents(
                            new TextInputBuilder().setCustomId('tag_req').setLabel('Tag requis ("COCO" ou vide)').setStyle(TextInputStyle.Short).setRequired(false)
                        ),
                    );
                await interaction.showModal(modal);
            ...
            // Participation Signal
                ...
                if (signal.tag_req && signal.tag_req.trim() !== '') {
                    ...
                    if (!hasCoreTag(member)) {
                        return await interaction.reply({ content: "Vous n'avez pas le tag requis. Le rôle `In The Circle` sert de validation COCO.", ephemeral: true });
                    }
                }
